...
export function createApp(database: Pick<Pool, 'query'>, overrides: Partial<AppConfig> = {}) {
  const app = express();
  ...
  app.use('/api/v1/auth', (request, response, next) => {
  ...
  app.use((error: unknown, _request: express.Request, response: express.Response, _next: express.NextFunction) => {
    void _next;
    const status = error instanceof AuthFailure ? error.status : error instanceof ZodError ? 400
      : error instanceof SyntaxError && 'body' in error ? 400 : 500;
    const code = error instanceof AuthFailure ? error.code : status === 400 ? 'INVALID_INPUT' : 'INTERNAL_ERROR';
    response.locals.errorCode = code;
    response.status(status).json({ error: {
      code, message: error instanceof AuthFailure ? error.message : status === 400 ? 'Invalid request' : 'Unexpected error',
